...
# Tissue
disease <- args[1]
# disease <- "Diabetes_mellitus_type_1"
tissue <- args[2]
# tissue <- "Pancreas"
...
outpath <- paste0(first_dir, "Jose/03_Models/Tissues/", tissue, "/")
...
# # PAR genes excluded
gene_annotation <- read.delim(paste0(first_dir, "Jose/00_Data/gencode.v26.GRCh38.genes.biotype_matched_v38.bed"))
...
# Metadata ----
metadata <- readRDS( paste0(outpath, "/", tissue,".SampleMetadata.", disease, ".rds"))
...
is.event.exprs <- function(event.id, tpm.threshold = 0.5){
  ...
  # ---- #
  # At least 20% of the samples express the most abundant isofoorm above 1 TPM ----
...
psi <- psi[,colnames(psi) %in% metadata$Sample]
...
if(grepl("Atherosclerotic", disease)){ #Metadata with too many variables
...
for(trait in individual_traits){
  if(trait=="Age" | trait == "BMI"){
    ...
    contrast_names <- c(contrast_names, "AncestryAFR")
  } else if(trait=="Sex"){
...
# -------------- #
print(Sys.time())  #Ha tardado 3 horas en hacer 1000 eventos de 19765, perdón? con un core, voy a hacer submit y habrá que ir poniendo varios cores
# -------------- #
events_exprs <- unlist(mclapply(rownames(psi), function(i) is.event.exprs(i, 0.5),  mc.cores = n_cores  ))
# -------------- #
print(Sys.time())
# -------------- #
save.image(file = paste0(first_dir, "/Jose/03_Models/Splicing/", disease, "_", tissue, "_test_data.RData"))
# load(paste0(first_dir, "/Jose/03_Models/Splicing/", disease, "_", tissue,"_test_data.RData"))
...
save.image(file = paste0(first_dir, "/Jose/03_Models/Splicing/", disease, "_", tissue, "_hier_part_test_data.RData"))
