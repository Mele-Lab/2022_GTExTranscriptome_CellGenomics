...
# spearman correlation between sample size and number/proportion of cis-driven DEGs
...
data <- cbind.data.frame("tissue" = rep(tissues, 2),
                         "value" = c(sapply(tissues, function(tissue) summary(ancestry_eGenes_DE[[tissue]][, "ieQTLs_R2"])[3]),
                         ...
                         "feature" = c(rep("eQTLs", length(tissues)), rep("Ancestry", length(tissues))))
...
wilcox.test(data[data$feature == "eQTLs", "value"],
            data[data$feature == "Ancestry", "value"],
            paired = T)


pdf(paste0(plot_path, "Figure_2_CEFG.pdf"),
    width = 9, height = 3)
...
pdf(paste0(plot_path, "Figure_2D.pdf"),
    width = 9, height = 4)
ggarrange(allele_frequency_plot, tpm_plot, widths = c(1,4))
dev.off()

pdf(paste0(plot_path, "Figure_2D.legend.pdf"),
    width = 4, height = 4)
plot.new()
legend("center",
       c("EA", "AA"),
